@echo off
title ONE MARKET - Sistema Definitivo (Terminal Abierto)
chcp 65001 >nul
cls
echo.
echo ╔════════════════════════════════════════════════════════════╗
...
echo.
echo ⏳ Este terminal permanecerá abierto para monitorear el sistema...
echo.

:monitor_loop
echo.
echo 📊 Estado del sistema:
echo    - API: Verificando...
python -c "
import requests
try:
...
try:
    ...
    if response.status_code == 200:
        print('    ✅ UI: FUNCIONANDO')
    else:
        print('    ❌ UI: ERROR')
except:
    print('    ❌ UI: NO DISPONIBLE')
"
