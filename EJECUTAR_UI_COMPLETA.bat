...
if not exist "ui\app_complete.py" (
    echo ❌ Error: No se encuentra ui\app_complete.py
    pause
    exit /b 1
)

echo ⏳ Iniciando UI Completa con todas las funcionalidades...
echo ℹ️  Presiona Ctrl+C para detener
echo.

set PYTHONPATH=%CD%

echo 🔧 Ejecutando: python -m streamlit run ui/app_complete.py
echo.

python -m streamlit run ui/app_complete.py
